...
build-all: build/uwsgi_exporter

build/uwsgi_exporter: FORCE
	go build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=uwsgi_exporter -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -o build/uwsgi_exporter .
...
install: FORCE build/uwsgi_exporter
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/uwsgi_exporter "$(DESTDIR)$(PREFIX)/bin/uwsgi_exporter"
...
build/cover.out: FORCE install-ginkgo | build
	...
	@env $(GO_TESTENV) ginkgo run --randomize-all -output-dir=build $(GO_BUILDFLAGS) -ldflags '-s -w -X github.com/sapcc/go-api-declarations/bininfo.binName=uwsgi_exporter -X github.com/sapcc/go-api-declarations/bininfo.version=$(BININFO_VERSION) -X github.com/sapcc/go-api-declarations/bininfo.commit=$(BININFO_COMMIT_HASH) -X github.com/sapcc/go-api-declarations/bininfo.buildDate=$(BININFO_BUILD_DATE) $(GO_LDFLAGS)' -covermode=count -coverpkg=$(subst $(space),$(comma),$(GO_COVERPKGS)) $(GO_TESTPKGS)
...
help: FORCE
	...
	@printf "  \e[36mbuild/uwsgi_exporter\e[0m            Build uwsgi_exporter\n"
